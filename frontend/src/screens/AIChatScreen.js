...
const dotStyles = StyleSheet.create({
  row: { flexDirection: "row", alignItems: "center", gap: 4, paddingVertical: 8, paddingHorizontal: 4 },
...
export function AIChatScreen({ route, navigation }) {
  ...
  const loadHistory = useCallback(async (sessionId) => {
    ...
    if (result.error) { setError(result.error || "Failed to load chat history."); return; }
  ...
  const renderMessage = ({ item, index }) => {
    ...
    const entering = isNew
      ? (isUser ? SlideInRight.duration(300) : SlideInLeft.duration(300))
      : undefined;
    ...
    return entering ? (
      <ReAnimated.View entering={entering} style={[styles.messageRow, isUser ? styles.messageRowUser : styles.messageRowAssistant]}>
        {bubbleContent}
      </ReAnimated.View>
    ) : (
...
const styles = StyleSheet.create({
  ...
  metaRow: { marginTop: 6, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 },
  suggestionsWrap: { flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 24, justifyContent: "center" },

...
{
    [Fact]
    public void MiddlewareOrder_CorsRegisteredBeforeRateLimiting()
    {
        ...
        var rateLimitIndex = source.IndexOf("UseRateLimiting");

        Assert.True(corsIndex > 0, "UseCors not found in Program.cs");
        Assert.True(rateLimitIndex > 0, "UseRateLimiting not found in Program.cs");
        Assert.True(corsIndex < rateLimitIndex,
            $"UseCors (index {corsIndex}) must appear before UseRateLimiting (index {rateLimitIndex}) in the middleware pipeline");
    }

    [Fact]
    public void MiddlewareOrder_AuthenticationRegisteredBeforeRateLimiting()
    {
        var programPath = Path.Combine(
            AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "Program.cs");
        var source = File.ReadAllText(programPath);
        ...
        Assert.True(authIndex < rateLimitIndex,
            $"UseAuthentication (index {authIndex}) must appear before UseRateLimiting (index {rateLimitIndex})");
    }
}
